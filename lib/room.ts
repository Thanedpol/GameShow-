...
/**
 * เฉลยกล่องคำใบ้ตอนจบข้อ — ว่ากล่องไหนจริง กล่องไหนหลอก และหลอกด้วยวิธีไหน
 *
 * ส่งข้ามเครื่องด้วย เพราะนี่คือส่วนที่เกมสอนจริง ๆ ถ้าเพื่อนไม่เห็น
 * ก็เล่นครบเกมโดยไม่เคยรู้เลยว่าที่เชื่อไปนั้นถูกหลอกตรงไหน
 * ขนาดราว 2KB ต่อข้อ เล็กกว่าภาพย่อที่ส่งอยู่แล้วหลายเท่า
 */
export interface RoomLiveReveal {
  id: string;
  label: string;
  text: string;
  truth: string;
...
export interface RoomLive {
  ...
  /** เวลาเต็มของช่วงนี้ (ms) — วงแหวนนับถอยหลังต้องรู้ ไม่งั้นวาดสัดส่วนไม่ได้ */
  durationMs?: number;
  boxes: RoomLiveBox[];
  /** เปิดไปกี่กล่องแล้ว — ใช้บอกโควตาที่เหลือให้ตรงกับจอเจ้าภาพ */
  openedCount?: number;
  /**
   * คะแนนที่เหลือของข้อนี้เป็น % หลังหักค่าเปิดกล่อง
   *
   * เจ้าภาพคิดแล้วส่งมา ไม่ให้จอเพื่อนคิดเอง เพราะการกาช่อง "ใช้โทเคน"
   * เป็น state ในจอเจ้าภาพเท่านั้น ถ้าคิดเองสองจอจะได้ตัวเลขไม่ตรงกัน
   */
  remainingPct?: number;
  /** เฉลยกล่องทั้งสี่ — มีเฉพาะตอนจบข้อแล้ว */
  reveal?: RoomLiveReveal[] | null;
  /**
   * จุดที่ทำได้ดี / จุดที่ควรปรับ จากผลตรวจของข้อนี้
   *
   * ไม่ได้อยู่ใน RoundResult ที่ไปกับสแนปช็อต จึงต้องส่งมาทางนี้
   * เป็นคำแนะนำที่ผู้เล่นเอาไปใช้ต่อได้ ไม่ควรมีแค่คนที่ถือรีโมตที่ได้อ่าน
   */
  strengths?: string[];
  improvements?: string[];
  /** กล่องไหนที่คนตอบเปิดไปจริง — ใช้ติดป้ายในหน้าเฉลย */
  openedIds?: string[];
